...
class ZShellFrame : public ZMDIChildFrame
{
	...
	void RedirectParent(HWND hwnd, HWND hwndExternal)
	{
		RECT rc;
		if (GetClientRect(hwnd, &rc))
		{
			if (hwnd = CreateShlWnd(hwnd, hwndExternal, rc.right - rc.left, rc.bottom - rc.top))
			{
				_hwndView = hwnd;
...
class CRun : public ZDlg
{
	...
	BOOL OnOk(HWND hwndDlg)
	{
		...
		if (0 <= hr)
		{
			...
			{
				if (hwnd = p->Create(sz, pExec))
				{
					ITask* pTask;
					if (0 <= (hr = pExec->EmbedTask(hwnd, &pi, &pTask)))
					{
						p->SetTask(pTask);
					}
				}
