...
struct IOPort : public IExecTask
{
	...
	{
		if (HWINEVENTHOOK hWinEventHook = SetWinEventHook(EVENT_OBJECT_CREATE, EVENT_OBJECT_FOCUS, 
	...
	virtual HRESULT STDMETHODCALLTYPE EmbedTask(_In_ HWND hwnd, _In_ PPROCESS_INFORMATION lpProcessInformation)
	{
		...
		if (ITask* pTask = new ITask(hwnd))
		{
			...
			{
				hr = AddParentInfo(hwnd, lpProcessInformation, pTask);
			}
		}
